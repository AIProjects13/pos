...
document.getElementById('btn-forgot-password').addEventListener('click', async () => {
    ...
    try {
        ...
        showToast("Enlace de recuperaciÃ³n enviado a tu correo.", "success");
    } catch (error) {
...
// Modificamos el closeModal genérico para que limpie el form si es el de producto
...
if(btnSaveProd) {
    btnSaveProd.addEventListener('click', async () => {
        ...
        try {
            ...
            showToast("Registrando en historial de auditoría...", "info");

            // 4. Registro de Gasto Automático
            let stockDiff = stock;
            if (isEdit) {
            ...
            // 5. Registro de Auditoría (Solo guardamos el Delta)
            const accion = isEdit ? 'Edición de Producto' : 'Creación de Producto';
...
window.initCategoriasListener = () => {
    unsubCategorias = db.collection("Categorias").onSnapshot(snapshot => {
        try {
            ...
            // Ordenar alfabéticamente protegiendo contra nombres indefinidos
...
function renderCategorias() {
    ...
    if (lst) {
        ...
        if (categoriasGlobal.length === 0) {
            lst.innerHTML = '<div class="text-center text-gray-400 py-4 text-sm">No hay categorías</div>';
        } else {
...
window.addCategoria = async () => {
    ...
    try {
        ...
        await logAuditChange('Inventario', 'Nueva Categoría', docId, `Creada categoría: ${nombre}`);
        input.value = '';
...
window.editCategoria = async (id, currentName) => {
    ...
    if (categoriasGlobal.find(c => c.id !== id && c.nombre.toLowerCase() === nombre.toLowerCase())) {
        showToast("Ya existe otra categoría con ese nombre", "error");
        return;
    }
    ...
    try {
        ...
        await logAuditChange('Inventario', 'Editar Categoría', id, `Categoría renombrada de ${currentName} a ${nombre}`);
...
window.deleteCategoria = async (id, nombre) => {
    if(confirm(`¿Eliminar la categoría "${nombre}"?`)) {
        try {
            ...
            await logAuditChange('Inventario', 'Eliminar Categoría', id, `Eliminada categoría: ${nombre}`);
...
window.procesarPago = async () => {
    ...
    try {
        ...
        cart.forEach(item => {
            ...
            // Usar FieldValue.increment para operaciones atómicas sería ideal, pero lo haremos manualmente por compatibilidad con el entorno actual
        ...
        // 3. Auditoría general
        await logAuditChange('Ventas', 'Nueva Venta', ventaId, `Venta por Q${posTotal.toFixed(2)} (${cart.length} artículos)`);
...
// ==========================================
// MÓDULO DE AUDITORÍA (ADMIN ONLY)
// ==========================================
...
// ==========================================
// MÓDULO DE USUARIOS Y ROLES (ADMIN ONLY)
// ==========================================
...
// Inicializar la app secundaria para creación de usuarios sin cerrar sesión
...
window.eliminarRol = async (uid, email) => {
    if(confirm(`¿Estás seguro de quitarle el acceso a ${email}? (Esto no borra su cuenta de Firebase, pero le impide entrar al sistema)`)) {
        try {
            await db.collection("Seguridad_Roles").doc(uid).delete();
            await logAuditChange('Seguridad', 'Eliminación de Rol', uid, `Se revocó acceso a: ${email}`);
...
if (btnSaveUsr) {
    btnSaveUsr.addEventListener('click', async () => {
        ...
        if(!email || pass.length < 6) {
            showToast("El correo es requerido y la contraseña debe tener al menos 6 caracteres", "error");
            return;
        }
        ...
        try {
            // Inicializar App Secundaria para evitar que cierre la sesión del Admin actual
...
window.guardarConfiguracion = async () => {
    ...
    try {
        ...
        await logAuditChange("Configuración", "Actualización General", "global", "Se actualizó la configuración visual de la tienda.");
    } catch (e) {
...
window.renderGastos = () => {
    ...
    sorted.forEach(g => {
        ...
        tr.innerHTML = 
            ...
            <td class="p-4">\</td>
            <td class="p-4">\</td>
            <td class="p-4 text-right font-bold text-red-600">Q\</td>
        ;
...
document.getElementById('btn-save-gas')?.addEventListener('click', async () => {
    ...
    try {
        await db.collection("Gastos").add(newGasto);
        registrarAuditoria('Gasto Registrado', \\: \ por Q\\);
...
window.abrirCierreCaja = () => {
    ...
    document.getElementById('cc-ventas-efectivo').innerText = \Q\\;
    document.getElementById('cc-ventas-tarjeta').innerText = \Q\\;
    document.getElementById('cc-gastos-dia').innerText = \- Q\\;
    document.getElementById('cc-esperado').innerText = \Q\\;
...
window.calcCierreDiferencia = () => {
    ...
    if(diff === 0) {
        ...
        diffBox.innerText = \Faltante: Q\\;
    } else {
        diffBox.classList.add('bg-blue-100', 'text-blue-700');
        diffBox.innerText = \Sobrante: Q\\;
    }
};
...
document.getElementById('btn-save-cierre')?.addEventListener('click', async () => {
    ...
    try {
        ...
        registrarAuditoria('Cierre de Caja', \Cierre realizado. Diferencia: Q\\);
